...
func TestStripEmptyTopicNameFromChangefeeds(t *testing.T) {
	...
	// CREATE CHANGEFEED requires a CCL binary which isn't available in this
	// test package, so we create changefeed jobs directly. The migration
	// operates on the persisted payload, which is identical regardless of
