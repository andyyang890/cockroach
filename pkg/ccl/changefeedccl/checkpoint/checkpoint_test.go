...
func TestCheckpointRestore(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		savedCheckpoint   *jobspb.TimestampSpansMap
		checkpointedSpans checkpointSpans
		...
		"restoring checkpoint with single timestamp": {
			...
			savedCheckpoint: jobspb.NewTimestampSpansMap(map[hlc.Timestamp]roachpb.Spans{
		...
		t.Run(name, func(t *testing.T) {
			...
			err = checkpoint.Restore(actualFrontier, tc.savedCheckpoint)
			if tc.expectedError != "" {
			...
			require.NoError(t, err)
			for _, s := range tc.checkpointedSpans {
